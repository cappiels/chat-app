...
CREATE TABLE IF NOT EXISTS knowledge_category_admins (
    ...
    appointed_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS knowledge_items (
    ...
    -- Temporal fields
    effective_date TIMESTAMPTZ, -- When this knowledge becomes effective
    expiry_date TIMESTAMPTZ, -- When this knowledge expires
    last_verified_at TIMESTAMPTZ, -- When this was last verified as accurate
...
-- Insert Default Knowledge Roles (PostgreSQL syntax)
...
-- PostgreSQL Triggers (different syntax from SQLite)
...
CREATE OR REPLACE FUNCTION update_knowledge_categories_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = CURRENT_TIMESTAMP;
    RETURN NEW;
...
CREATE TRIGGER update_knowledge_categories_timestamp 
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_knowledge_categories_timestamp();
...
CREATE TRIGGER update_knowledge_tag_usage 
    AFTER INSERT ON knowledge_item_tags
    FOR EACH ROW
    EXECUTE FUNCTION update_knowledge_tag_usage();
...
CREATE TRIGGER update_knowledge_views_count 
    AFTER INSERT ON knowledge_analytics
    FOR EACH ROW
    EXECUTE FUNCTION update_knowledge_views_count();
