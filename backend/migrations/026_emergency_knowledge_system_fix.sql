-- EMERGENCY FIX: Handle knowledge system migration failures
-- This migration fixes the dependency issues with knowledge tables

-- First, drop all knowledge-related objects with CASCADE to handle dependencies
...
DROP TABLE IF EXISTS knowledge_item_scopes CASCADE;
DROP TABLE IF EXISTS knowledge_items CASCADE;
DROP TABLE IF EXISTS knowledge_category_admins CASCADE;
DROP TABLE IF EXISTS knowledge_categories CASCADE;
DROP TABLE IF EXISTS knowledge_scope_members CASCADE;
DROP TABLE IF EXISTS knowledge_roles CASCADE;
DROP TABLE IF EXISTS knowledge_scopes CASCADE;

-- Drop any remaining views that might have been created
DROP VIEW IF EXISTS knowledge_views CASCADE;

-- Drop any functions/triggers that might exist
DROP FUNCTION IF EXISTS update_knowledge_items_timestamp() CASCADE;
DROP FUNCTION IF EXISTS update_knowledge_categories_timestamp() CASCADE;
DROP FUNCTION IF EXISTS update_knowledge_tag_usage() CASCADE;
DROP FUNCTION IF EXISTS update_knowledge_views_count() CASCADE;
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;

-- Now recreate the knowledge system properly (based on migration 010 which has better PostgreSQL syntax)
...
CREATE TABLE IF NOT EXISTS knowledge_categories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL,
    scope_id UUID, -- Categories can be scoped to specific knowledge scopes
    name VARCHAR(255) NOT NULL,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS knowledge_items (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL,
    primary_scope_id UUID NOT NULL, -- Main scope this belongs to
    created_by VARCHAR(128) NOT NULL, -- Firebase UID
...
CREATE TABLE IF NOT EXISTS knowledge_tags (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL,
    scope_id UUID, -- Tags can be scoped
    name VARCHAR(100) NOT NULL,
    description TEXT,
    color VARCHAR(7),
...
CREATE TABLE IF NOT EXISTS knowledge_analytics (
    ...
    scope_id UUID, -- Which scope they accessed it from
