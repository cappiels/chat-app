...
<body>
  ...
  async sendWorkspaceAddedNotification({
    ...
    // Create a simple HTML email for workspace addition
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>You've been added to ${workspaceName}</title>
    <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #f5f5f5; }
        .container { max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        .header { background: linear-gradient(135deg, #00b894 0%, #00cec9 100%); color: white; padding: 40px 30px; text-align: center; }
        .logo { font-size: 32px; font-weight: bold; margin-bottom: 10px; }
        .content { padding: 40px 30px; }
        .workspace-info { background: #f0fff4; padding: 25px; border-radius: 8px; margin: 25px 0; border-left: 4px solid #00b894; }
        .cta-button { display: inline-block; background: linear-gradient(135deg, #00b894 0%, #00cec9 100%); color: white; padding: 16px 32px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px; margin: 25px 0; }
        .footer { background: #f8f9fa; padding: 30px; text-align: center; font-size: 14px; color: #666; }
        .celebration { font-size: 48px; margin: 20px 0; }
    </style>
...
<body>
    <div class="container">
        <div class="header">
            <div class="celebration">🎉</div>
            <div class="logo">💬 crew</div>
            <h1>You've been added to ${workspaceName}!</h1>
        </div>
        
        <div class="content">
            <p>Great news! <strong>${adminName}</strong> has added you to the <strong>${workspaceName}</strong> workspace.</p>
            
            <div class="workspace-info">
                <h3 style="margin-top: 0; color: #00b894;">${workspaceName}</h3>
                <p style="margin-bottom: 10px;">${workspaceDescription || 'A collaborative workspace for your team'}</p>
                <p style="margin: 0; color: #666; font-size: 14px;">
                    Your role: <strong>${userRole}</strong>
                </p>
            </div>
            
            <p>You now have access to this workspace. Simply log in to start collaborating with your team!</p>
            
            <div style="text-align: center;">
                <a href="${workspaceUrl}" class="cta-button">Go to ${workspaceName}</a>
            </div>
            
            <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
            
            <div style="font-size: 14px; color: #666;">
                <p><strong>What can you do in crew?</strong></p>
                <ul style="margin: 0; padding-left: 20px;">
                    <li>Real-time team messaging and collaboration</li>
                    <li>File sharing and threaded conversations</li>
                    <li>Task management and calendar integration</li>
        ...
        <div class="footer">
            ...
            <p style="margin: 10px 0 0 0;">
                <a href="${workspaceUrl}" style="color: #00b894;">Open ${workspaceName}</a>
            </p>
        </div>
    </div>
</body>
    ...
    return this.sendEmail({
      to,
      subject: `🎉 You've been added to ${workspaceName} on crew`,
      html,
      category: 'workspace-added'
    });
  }
